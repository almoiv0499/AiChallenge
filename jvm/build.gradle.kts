...
// Задача для запуска деплоя на Railway
tasks.register<JavaExec>("deployRailway") {
    group = "deployment"
    description = "Деплоит приложение на Railway"
    mainClass.set("org.example.deployment.DeployMainKt")
    classpath = sourceSets["main"].runtimeClasspath
    
    // Передаём аргументы из командной строки или переменных окружения
    if (project.hasProperty("railwayToken")) {
        args("--railway-token=${project.property("railwayToken")}")
    }
    if (project.hasProperty("projectId")) {
        args("--project-id=${project.property("projectId")}")
    }
    if (project.hasProperty("serviceId")) {
        args("--service-id=${project.property("serviceId")}")
    }
}